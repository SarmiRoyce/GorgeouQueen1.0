...
function Profile() {
  ...
  const [showServicesForm, setShowServicesForm] = useState(false);  const handleChange = (e) => {
    const { name, value, type, checked, files } = e.target;
    if (type === 'file') {
      const file = files[0];
      const imageUrl = URL.createObjectURL(file);
    ...
    e.preventDefault();    // Check if at least one service is selected
    ...
    <div className="pro-form5">
      <div className="pro-form"><br />
        <h1 style={{ textAlign: 'center', fontWeight: '600' }}><b>Explore Your Profile</b></h1>        <form onSubmit={handleSubmit}>
          <div className="profile-header">
            {formData.imageUrl && <img src={formData.imageUrl} alt="Beautician's" style={{ width: '150px', height: '150px', borderRadius: '50%' }} />}
            <input
              type="file"
